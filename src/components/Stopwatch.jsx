...
const Card = () => {
    const [count, setCount] = useState(() => parseInt(localStorage.getItem('stopwatch')) || 0);
    const [isRunning, setIsRunning] = useState(false);

    useEffect(() => {
        localStorage.setItem('stopwatch', count);
    }, [count]);
    ...
    return (
        <StyledWrapper>
            <div className="watch">
                <div className="frame">
                    <div className="text">
                        <div className="time">{String(Math.floor(count / 100)).padStart(2, '0')}</div>
                        <div className="time">{String(count % 100).padStart(2, '0')}</div>
                    </div>
                </div>
